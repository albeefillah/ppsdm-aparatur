@section('title') 
PPSDM Aparatur - Edit Sub Komponen
@endsection 
        ...
        <div class="col-md-6 col-lg-6">
            <div class="xp-breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Edit Sub Komponen</li>
                </ol>
            </div>
        </div>
    </div>          
    ...
    <div class="col-lg-12">
        <div class="card m-b-30">
            <div class="card-header bg-white">
                <h5 class="card-title text-black">Edit Sub Komponen</h5>
                <h6 class="card-subtitle">Ubah form dibawah ini.</h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('sub-komponen.update', $subKomponen->id) }}" onsubmit="return Validation()" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                      ...
                      <select name="id_rincian_output" id="id_rincian_output" class="form-control xp-select2-single" required>
                        ...
                        @foreach ($ro as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $subKomponen->id_rincian_output ? 'selected' :'' }}>{{ $item->kro->kegiatanProgram->kode }}.{{ $item->kro->kode }}.{{ $item->kode }} - {{ $item->deskripsi }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group">
                      <label for="kode">Kode *)</label>
                      <input type="text" required name="kode" class="form-control" id="kode" value="{{ $subKomponen->kode }}" >
                    </div>
                    <div class="form-group">
                      <label for="deskripsi">Deskripsi *)</label>
                      <input type="text" class="form-control" name="deskripsi" id="deskripsi" value="{{ $subKomponen->deskripsi }}"  required>
                    </div>
                    <div class="form-group">
                        <label for="pagu_awal">Pagu Awal</label>
                        <input types="number" name="pagu_awal" min="0" class="form-control" id="pagu_awal" value="{{ $subKomponen->pagu_awal }}">
                    </div>
