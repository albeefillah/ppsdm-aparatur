@section('title') 
PPSDM Aparatur - Edit Rincian Output
@endsection 
        ...
        <div class="col-md-6 col-lg-6">
            <div class="xp-breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Edit Rincian Output</li>
                </ol>
            </div>
        </div>
    </div>          
    ...
    <div class="col-lg-12">
        <div class="card m-b-30">
            <div class="card-header bg-white">
                <h5 class="card-title text-black">Edit Rincian Output</h5>
                <h6 class="card-subtitle">Ubah form dibawah ini.</h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('rincian-output.update', $ro->id) }}" onsubmit="return Validation()" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                      ...
                      <select name="id_kro" id="id_kro" class="form-control xp-select2-single" required>
                        ...
                        @foreach ($kro as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $ro->id_kro ? 'selected' : '' }}>{{ $item->kegiatanProgram->kode }}.{{ $item->kode }} - {{ $item->deskripsi }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group">
                      <label for="kode">Kode *)</label>
                      <input type="text" required name="kode" class="form-control" id="kode" value="{{ $ro->kode }}">
                    </div>
                    <div class="form-group">
                      <label for="deskripsi">Deskripsi *)</label>
                      <input type="text" class="form-control" name="deskripsi" id="deskripsi" value="{{ $ro->deskripsi }}" required>
                    </div>
                    <div class="form-group">
                        <label for="pagu_awal">Pagu Awal</label>
                        <input types="number" name="pagu_awal" min="0" class="form-control" id="pagu_awal" value="{{ $ro->pagu_awal }}">
                    </div>
