<?php
// 4 kerja 2 libur dengan 19 job ,6 job rotatable
...
class GenerateMonthlySchedulesw extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($datesArray as $date) {
            ...
            // Pisahkan job rotatable dan tetap
            $rotatableJobs = $allJobs->where('rotatable', true)->values();
            $fixedJobs = $allJobs->where('rotatable', false)->values();

            // Bagi rotatable menjadi 2 grup: 3 job per hari
            $rotGroup1 = $rotatableJobs->filter(fn($job, $i) => $i % 2 === 0);
            $rotGroup2 = $rotatableJobs->filter(fn($job, $i) => $i % 2 === 1);
            $activeRotatable = $date->day % 2 === 0 ? $rotGroup1 : $rotGroup2;

            $jobsToday = $fixedJobs->merge($activeRotatable)->values();
            ...
            foreach ($nightJobs as $job) {
                ...
                $selectedEmployee = $eligibleEmployees->firstWhere(function ($emp) use ($job, $workingEmployees, $jobsToday, $dateStr) {
                    $otherJobs = $jobsToday->filter(fn($j) => $j->id !== $job->id);
        ...
        $this->info("\u2705 Jadwal {$month}/{$year} berhasil dibuat dengan rotasi 6 job harian (3 aktif per hari).");
    }
