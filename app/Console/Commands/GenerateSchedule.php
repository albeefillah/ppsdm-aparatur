...
class GenerateMonthlySchedule extends Command
{
    ...
    protected function generateScheduleForEmployee($employee, $dates, $holidays)
    {
        ...
        $firstExistingSchedule = Schedule::where('employee_id', $employee->id)
            ->orderBy('work_date', 'asc')
            ->first();

        $offset = 0;

        if ($firstExistingSchedule) {
            $firstDate = Carbon::parse($firstExistingSchedule->work_date);
            $diffDays = $firstDate->diffInDays(Carbon::create($firstDate->year, $firstDate->month)->startOfMonth());
            $offset = $diffDays % 6;
        } else {
        ...
        foreach ($dates as $date) {
            ...
            $adjustedIndex = ($dayIndex + $offset) % 6;
